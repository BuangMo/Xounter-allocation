...
class TSP_GENETIC_ALGORITHM():
    ...
    def crossover1Point(self, par1, par2):
        '''Performs 1 point crossover method using for the generation of the child'''
        ...
        for i in range(point_sel):
            # search for the genetic material of parent 2 in the offspring
            try:
    ...
    def mutation(self, child_dna):
        ...
        for i in range(self.num_locations):
            probs = random.random()
            if probs >= prob:
                x_possibilities = [num for num in range(self.num_locations) if num != i]
